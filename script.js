    ...
    try {
        ...
        try {
            ...
            try {
                ...
                // Если все еще ошибка, проверяем текстовый ответ
                if (text.includes('SUCCESS') || text.includes('Успех')) {
                    return { status: 'success', message: 'Заявка отправлена' };
                } else if (text.includes('ERROR') || text.includes('Ошибка')) {
                    return { 
                        status: 'error', 
                        message: text.replace('ERROR:', '').replace('Ошибка:', '').trim() || 'Ошибка отправки' 
                    };
                } else {
                    return {
                        status: 'error',
...
orderForm.addEventListener('submit', async function(e) {
    ...
    try {
        ...
        formData.append('phone', phoneForServer); // Отправляем очищенный номер
